...
class Master(Script):
  def install(self, env):
    ...
    if params.setup_prebuilt:
      ...
      Execute('cd '+params.flink_install_dir+'; git --branch release-1.7.0 clone https://github.com/apache/flink.git '+params.flink_install_dir +' >> ' + params.flink_log_file)
  ...
  def start(self, env):
    ...
    cmd = format("export HADOOP_CLASSPATH=$(hadoop classpath) export HADOOP_CONF_DIR={hadoop_conf_dir}; {bin_dir}/yarn-session.sh -n {flink_numcontainers} -s {flink_numberoftaskslots} -jm {flink_jobmanager_memory} -tm {flink_container_memory} -qu {flink_queue} -nm {flink_appname} -d")
